...
public class RelationExtraction {
	...
	String apiURL = "/calls/text/TextGetRelations";
	
	@POST
	@Consumes(MediaType.APPLICATION_FORM_URLENCODED)
	@Produces(MediaType.APPLICATION_XML)
	public String TextGetRelations(@FormParam("text") String text,
			@FormParam("url") String url,
			@FormParam("maxRetrieve") String maxRetrieve,
			@FormParam("sentiment") String sentiment,
			@FormParam("entities") String entities,
			@FormParam("keywords") String keywords,
			@FormParam("requireEntities") String requireEntities,
			@FormParam("sentimentExcludeEntities") String sentimentExcludeEntities,
		...
		try {
			...
			byte[] requestData = buildHttpPostData(text, apikey, url,
					maxRetrieve, sentiment, entities, keywords, requireEntities,
					sentimentExcludeEntities, disambiguate, linkedData, coreference, showSourceText);
			...
			try {
				...
				return xmlHeader + RelationExtraction.buildXmlResponse(xmlBuilder, "exception",
						e.getClass().getName());
			} catch (Exception e1) {
	...
	private static byte[] buildHttpPostData(String text, String serviceApiKey,
			String url, String maxRetrieve, String sentiment, String entities, String keywords,
			String requireEntities, String sentimentExcludeEntities, String disambiguate, String linkedData, 
			String coreference, String showSourceText) throws UnsupportedEncodingException {
		...
		if (entities == null) {
			parameterMap.put("entities", 0);
		} else {
			parameterMap.put("entities", entities);
		}
		if (keywords == null) {
			parameterMap.put("keywords", 0);
		} else {
			parameterMap.put("keywords", keywords);
		}
		if (requireEntities == null) {
			parameterMap.put("requireEntities", 0);
		} else {
			parameterMap.put("requireEntities", requireEntities);
		}
		if (sentimentExcludeEntities == null) {
			parameterMap.put("sentimentExcludeEntities", 1);
		} else {
			parameterMap.put("sentimentExcludeEntities", sentimentExcludeEntities);
		}
		...
		return RelationExtraction.encodeHttpPostParameters(parameterMap);
	}
