...
public class KeywordExtraction {
	...
	public String TextGetRankedKeywords(@FormParam("text") String text,
			@FormParam("url") String url,
			@FormParam("maxRetrieve") String maxRetrieve,
			@FormParam("keywordExtractMode") String keywordExtractMode,
		...
		try {
			...
			byte[] requestData = buildHttpPostData(text, apikey, url,
					maxRetrieve, keywordExtractMode, sentiment, showSourceText,
					knowledgeGraph);
			...
			try {
				...
				return xmlHeader + KeywordExtraction.buildXmlResponse(xmlBuilder, "exception",
						e.getClass().getName());
			} catch (Exception e1) {
	...
	private static byte[] buildHttpPostData(String text, String serviceApiKey,
			String url, String maxRetrieve, String keywordExtractMode, String sentiment, String showSourceText, 
			String knowledgeGraph) throws UnsupportedEncodingException {
		...
		if (keywordExtractMode == null) {
			parameterMap.put("keywordExtractMode", "normal");
		} else {
			parameterMap.put("keywordExtractMode", keywordExtractMode);
		}
		if (sentiment == null) {
		...
		return KeywordExtraction.encodeHttpPostParameters(parameterMap);
	}
