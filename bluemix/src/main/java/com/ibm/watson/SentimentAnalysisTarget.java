...
@Path("/targetsentiment")
public class SentimentAnalysisTarget {
	String ORIGINAL_SERVICE_URL = "https://access.alchemyapi.com";
	String xmlHeader = "<?xml version='1.0' encoding='UTF-8'?>";
	String apiURL = "/calls/text/TextGetTargetedSentiment";
	
	@POST
	@Consumes(MediaType.APPLICATION_FORM_URLENCODED)
	@Produces(MediaType.APPLICATION_XML)
	public String TextGetTargetedSentiment(@FormParam("text") String text,
		...
		try {
			...
			byte[] requestData = buildHttpPostData(text, apikey, target, url, showSourceText);
			...
			try {
				...
				return xmlHeader + SentimentAnalysisTarget.buildXmlResponse(xmlBuilder, "exception",
						e.getClass().getName());
			} catch (Exception e1) {
	...
	private static byte[] buildHttpPostData(String text, String serviceApiKey, String target,
		...
		return SentimentAnalysisTarget.encodeHttpPostParameters(parameterMap);
	}
