...
public class Taxonomy {
	...
	String apiURL = "/calls/text/TextGetRankedTaxonomy";
	
	@POST
	@Consumes(MediaType.APPLICATION_FORM_URLENCODED)
	@Produces(MediaType.APPLICATION_XML)
	public String TextGetRankedTaxonomy(@FormParam("text") String text,
		...
		try {
			...
			try {
				...
				return xmlHeader + Taxonomy.buildXmlResponse(xmlBuilder, "exception",
						e.getClass().getName());
			} catch (Exception e1) {
	...
	private static byte[] buildHttpPostData(String text, String serviceApiKey, 
		...
		return Taxonomy.encodeHttpPostParameters(parameterMap);
	}
